...
Array.prototype.distinct = function () {
    return this.where(function (item, index, iterator) { return iterator.indexOf(item) === index; });
};
...
Array.prototype.contains = function (object) {
    return this.some(function (item) { return item === object; });
};
Array.prototype.except = function (source) {
    return this.where(function (item) { return !source.contains(item); });
};
...
Array.prototype.sum = function (expression) {
    return expression ? this.select(expression).sum() : this.aggregate(function (accumulator, value) { return accumulator += (+value); }, 0);
};
...
Array.prototype.groupBy = function (group, expression) {
    return this.aggregate(function (accumulator, value) { return ((accumulator)[group(value)]
...
Array.prototype.max = function () {
    return this.aggregate(function (workingItem, nextItem) { return workingItem > nextItem ? workingItem : nextItem; });
};
...
Array.prototype.zip = function (second, resultSelector) {
    var until = Math.min(second.length, this.length);
    var result = [];
...
Array.prototype.groupJoin = function (inner, outerKeySelector, innerKeySelector, resultSelector) {
    if (this.any() || inner.any()) {
        return this.select(function (x, y) { return resultSelector(x, inner.where(function (a) { return outerKeySelector(x) === innerKeySelector(a); })); });
    }
...
Array.prototype.takeWhile = function (predicate) {
    ...
    if (this.any() && predicate !== undefined) {
        return this.take(this.aggregate(function (x, y) { return predicate(_this.elementAt(x)) ? ++x : x; }, 0));
    }
...
//# sourceMappingURL=linq-to-type.js.map